...
export default function Post({ url }) {
      ...
      if (fetchedPost) {
        try {
          ...
          const response = await fetch(
            likes.userLiked
              ? `/api/v1/likes/?postid=${data.postid}`
              : likes.url,
            {
              method: likes.userLiked ? "POST" : "DELETE",
  ...
  const likeUnlike = () => {
    setLikes((prev) => ({
      ...
      num: prev.userLiked ? prev.num - 1 : prev.num + 1,
    }));
    ...
    (async () => {
      if (fetchedPost) {
        try {
          let bool = false;
          const response = await fetch(
            `/api/v1/comments/?postid=${data.postid}`,
            {
              method: "POST",
  ...
  useEffect(() => {
    (async () => {
      if (fetchedPost) {
        try {
          let bool = false;
          const response = await fetch(`/api/v1/comments/${cmtToDel}/`, {
            method: "DELETE",
            credentials: "same-origin",
          });
          if (!response.ok) throw Error(response.statusText);
          const getUpdatedCmts = await fetch(`api/v1/posts/${data.postid}/`, {
            credentials: "same-origin",
          });
    ...
    (async () => {
      if (fetchedPost && doubleClickable && doubleClicked) {
        try {
          ...
          let bool = false;
          const response = await fetch(`/api/v1/likes/?postid=${data.postid}`, {
            method: "POST",
            credentials: "same-origin",
          });
          if (!response.ok) throw Error(response.statusText);
      ...
      <div className="post index">
        ...
        <div className="content">
          <div className="pic" onDoubleClick={doubleClickToLike}>
            <img
              src={data.imgUrl}
              alt={`${data.owner}'s ${data.created} post`}
            />
          </div>

...
from insta485.views.db_scripts import (
    JUST_INSERTED_ID, GET_USER_FULL_INFO, GET_LIKER, GET_LIKEID_FROM_LIKER,
    GIVE_LIKE, GET_LIKER_FROM_LIKEID, REMOVE_LIKE_BY_LIKEID, NEW_COMMENT,
    GET_COMMENT, DEL_COMMENT, GET_COMMENT_INFO_PER_POST, GET_POST_FULL_INFO,
    GET_RESULTS)
...
def handle_error(err: Error) -> Response:
    """Handle an error."""
    response = jsonify({"message": err.message, "status_code": err.status})
...
def verify_password(logname: str, auth_pw: str) -> bool:   # Helper function
    """Verify password."""
    _, the_salt, real_pw = model.get_db().cursor().execute(
        GET_USER_FULL_INFO, (logname,)).fetchone()["password"].split('$')
...
def check_authentication_error(postid_oor=False, postid=0) -> None:
    ...
    if (auth is None and "logname" not in session) or (
        auth is not None and not verify_password(
            auth["username"], auth["password"])):
        raise Error("Forbidden", 403)
    if postid_oor and model.get_db().cursor().execute(
            GET_POST_FULL_INFO, (postid, )).fetchone() is None:
...
def get_index():
    ...
    username: str = session[
        "logname"] if request.authorization is None else (
        request.authorization['username'])
    cursor: Cursor = model.get_db().cursor()
    most_recent: int = cursor.execute(
        f"{GET_RESULTS} LIMIT 1;", (
            username, username, float("inf"))).fetchone()["postid"]
    postid_lte: int = request.args.get(
        "postid_lte", default=most_recent, type=int)
    ...
    if (page is not None and page < 0) or (size is not None and size < 0):
        raise Error("Bad Request", 400)
    url = urlparse(request.url)
    context: dict = {
        "next": "", "url": f"{url.path}?{url.query}" if (
            len(url.query) != 0) else url.path}
    context["results"] = cursor.execute(
        f"{GET_RESULTS} LIMIT {size} OFFSET {page * size};",
        (username, username, postid_lte)).fetchall()
...
def get_post(postid: int) -> Response:
    ...
    cursor: Cursor = model.get_db().cursor()
    context = {
        "postShowUrl": f"/posts/{postid}/", "postid": postid,
    ...
    context["ownerImgUrl"] = "/uploads/" + cursor.execute(
        GET_USER_FULL_INFO, (poster, )).fetchone()["filename"]
    comments: list[dict] = cursor.execute(
    ...
    logname_likes_this: bool = {"owner": username} in likers
    likes: dict = {
        "lognameLikesThis": logname_likes_this, "numLikes": len(likers),
        "url": None if not logname_likes_this else "/api/v1/likes/"}
    if logname_likes_this:
        likeid: str = str(cursor.execute(
            GET_LIKEID_FROM_LIKER, (
                username, postid)).fetchone()["likeid"]) + "/"
        likes["url"] += likeid
    context["likes"] = likes
...
def give_like() -> Response:
    ...
    if {"owner": username} in likers:
        likeid: int = cursor.execute(
            GET_LIKEID_FROM_LIKER, (username, postid)).fetchone()["likeid"]
        return jsonify(likeid=likeid, url=f"/api/v1/likes/{likeid}/"), 200
    cursor.execute(GIVE_LIKE, (username, postid))
    likeid = cursor.execute(
        JUST_INSERTED_ID).fetchone()["last_insert_rowid()"]
    conn.commit()
    conn.close()
...
def give_comment() -> Response:
    ...
    commentid: int = cursor.execute(
        JUST_INSERTED_ID).fetchone()["last_insert_rowid()"]
    conn.commit()
    conn.close()
    return jsonify(
        commentid=commentid, lognameOwnsThis=True, owner=username,
        ownerShowUrl=f"/users/{username}/", text=comment,
        url=f"/api/v1/comments/{commentid}/"), 201
